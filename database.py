...
def save_search(
    ...
    """Save weather search record and return new record_id"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def update_record(
    ...
    """Update temperature and/or condition of an existing record"""
    conn = get_db_connection()
    cursor = conn.cursor()
...
def export_records() -> List[Dict]:
    """Export all records ordered by latest first"""
    conn = get_db_connection()
    cursor = conn.cursor()
